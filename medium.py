...
# New Models
# ==================
def predict_zlema(prices, period=10):
    ...
    return safe_return(alpha * adj + (1 - alpha) * z)

def predict_covwma(prices, p=12):
    if len(prices) < p:
        return None

    arr = np.array(prices[-p:])
    r = log_returns(arr)
    cv = np.std(r) / (np.mean(arr) + 1e-8)
    w = np.exp(-cv * np.arange(p)[::-1])
    w /= w.sum()

    return safe_return(np.sum(arr * w))

def predict_t3(prices, period=10, v=0.7):
    if len(prices) < period * 3:
        return None
    ...
    return safe_return(c1*e3 + c2*e2 + c3*e1 + c4*prices[-1])
...
def predict_ichimoku(prices):
    ...
    return safe_return(span)

def predict_parma(prices, period=12):
    if len(prices) < period + 3:
        return None

    r = log_returns(np.array(prices))
    phase = np.tanh(np.mean(r[-period:]) * 100)

    alpha = np.clip(0.1 + abs(phase), 0.1, 0.9)

    p = prices[0]
    for x in prices[1:]:
        p = alpha * x + (1 - alpha) * p

    return safe_return(p)
