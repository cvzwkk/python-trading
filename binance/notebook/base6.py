...
# =========================
# LIVE STREAM LOOP WITH BUY/SELL AND SIMULATION
# =========================
async def depth_stream():
    global last_signal, USD_balance, BTC_balance, trade_cache
    print("🔵 Complete High-Frequency Predictive Engine with Simulated PnL\n")
    async with websockets.connect(WS_URL) as ws:
        async for msg in ws:
            ...
            # ===== BUY / SELL ALERT LOGIC WITH SIMULATION =====
            signal_alert = None
            if trend_final == "BULLISH 📈" and last_signal != "BUY":
                usd_to_spend = USD_balance * trade_size_pct
                ...
                signal_alert = f"💰 BUY ALERT! Entry Price: {midprice:.2f} USD | Bought: {btc_to_buy:.6f} BTC"
                last_signal = "BUY"

            elif trend_final == "BEARISH 📉" and last_signal != "SELL":
                btc_to_sell = BTC_balance * trade_size_pct
                ...
                signal_alert = f"📉 SELL ALERT! Entry Price: {midprice:.2f} USD | Sold: {btc_to_sell:.6f} BTC"
                last_signal = "SELL"
