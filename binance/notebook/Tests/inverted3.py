...
# Intraday Predict
sec_returns = deque(maxlen=1800)     # up to 30 minutes of 1s returns
sec_prices  = deque(maxlen=1800)
micro_press = deque(maxlen=1800)
...
# Intraday Predict
def predict_tf_open(tf_seconds):
    """
    Predict future candle OPEN for any timeframe using 1s data.
    tf_seconds: 60, 180, 300, 900 ...
    Returns: open, tp, sl, confidence
    """
    lookback = min(len(sec_returns), max(120, tf_seconds * 2))
    if lookback < 60:
        return None, None, None, 0.0

    ret = np.array(list(sec_returns)[-lookback:])
    press = np.array(list(micro_press)[-lookback:])

    # --- pressure-weighted drift
    weights = np.tanh(np.abs(press)) + 1e-6
    drift = np.average(ret, weights=weights)

    # --- forward projection
    expected_move = drift * tf_seconds
    current_price = sec_prices[-1]
    predicted_open = current_price + expected_move

    # --- realized volatility scaled
    vol = np.std(ret) * np.sqrt(tf_seconds)

    # --- dynamic targets
    tp = predicted_open + vol * 1.25
    sl = predicted_open - vol * 1.25

    # --- confidence score
    confidence = min(1.0, abs(expected_move) / (vol + 1e-9))
...
# Intraday Predict
            ...
            if len(sec_prices) > 0:
                sec_returns.append(midprice - sec_prices[-1])

            sec_prices.append(midprice)

            micro_press.append(
                weighted_imbalance() +
                order_flow_imbalance() +
                micro_momentum(midprice)
            )

            pred_1m  = predict_tf_open(60)
            pred_3m  = predict_tf_open(180)
            pred_5m  = predict_tf_open(300)
            pred_15m = predict_tf_open(900)
            ...
            now = datetime.utcnow()
            print("\n⏱", now, "UTC")
            print("🧭 MULTI-TIMEFRAME CANDLE OPEN FORECASTS")

            def print_tf(label, data):
                o, tp, sl, c = data
                if o:
                    print(f" {label:4} | Open: {o:.2f} | TP: {tp:.2f} | SL: {sl:.2f} | Conf: {c*100:5.1f}%")
                else:
                    print(f" {label:4} | Warming up...")
            print_tf("1m",  pred_1m)
            print_tf("3m",  pred_3m)
            print_tf("5m",  pred_5m)
            print_tf("15m", pred_15m)
            print("------------------------------------------------------------")

# =========================
# TREND SIGNALS (SIDE-BY-SIDE)
# =========================
            print("⭐ Trend Models Signals (Inverted)")

            items = list(signals_dict.items())
            for i in range(0, len(items), 2):
                left = items[i]
                right = items[i+1] if i+1 < len(items) else None
                line = f" {left[0]:4} | Signal: {left[1]:>4}"
                if right:
                    line += f"    {right[0]:4} | Signal: {right[1]:>4}"
                print(line)

            print("------------------------------------------------------------")

            print("⭐ Trend Models Signals (Normal)")

            items_normal = list(signals_dict_normal.items())
            for i in range(0, len(items_normal), 2):
                left = items_normal[i]
                right = items_normal[i+1] if i+1 < len(items_normal) else None
                line = f" {left[0]:4} | Signal: {left[1]:>4}"
                if right:
                    line += f"    {right[0]:4} | Signal: {right[1]:>4}"
                print(line)
