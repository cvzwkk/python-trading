...
async def depth_stream():
    ...
    print("🔵 Complete High-Frequency Predictive Engine (Python)\n")
    async with websockets.connect(WS_URL) as ws:
        async for msg in ws:
            ...
            if cross_signal == "BUY 🟢":
                if position != "LONG":
                    if position == "SHORT":
                        pnl = entry_price - midprice
                        balance += pnl
                        trade_log.append({"side":"CLOSE SHORT", "price":midprice, "pnl":pnl, "balance":balance})
                    position = "LONG"
                    entry_price = midprice
                    trade_log.append({"side":"OPEN LONG", "price":midprice, "balance":balance})
            elif cross_signal == "SELL 🔴":
                if position != "SHORT":
                    if position == "LONG":
                        pnl = midprice - entry_price
                        balance += pnl
                        trade_log.append({"side":"CLOSE LONG", "price":midprice, "pnl":pnl, "balance":balance})
                    position = "SHORT"
                    entry_price = midprice
                    trade_log.append({"side":"OPEN SHORT", "price":midprice, "balance":balance})
            ...
            if position:
                unrealized = (midprice-entry_price) if position=="LONG" else (entry_price-midprice)
                print(f"   Current Position: {position} | Entry: {entry_price:.2f} | Unrealized PnL: {unrealized:.2f} | Balance: {balance:.2f}")
            else:
                print(f"   No open position | Balance: {balance:.2f}")
            print("⭐ Last Trades:")
            for t in trade_log[-5:]:
                print(f"   {t['side']} @ {t['price']:.2f} | PnL: {t.get('pnl',0):.2f} | Balance: {t['balance']:.2f}")
            print("------------------------------------------------------------")
